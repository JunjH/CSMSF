...
parser = argparse.ArgumentParser(description='training on lidar-thermal')
...
parser.add_argument('--name', default='model_lt', type=str, help='name of experiment')
...
def main():
    ...
    model_lidar = net.model(encoder_lidar, block_channel=[24, 32, 96, 160])
    model_lidar.to(device)
    checkpoint = torch.load("./runs/model_l.pth.tar")
    model_lidar.load_state_dict(checkpoint2['state_dict'])
    ...
    model_thr.load_state_dict(checkpoint3['state_dict'])
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train(train_loader, model_lidar, model_thr, model, optimizer)
        total_score = test(test_loader, model_lidar, model_thr, model)
...
def train(train_loader, model_lidar, model_thr, net, optimizer):
    net.train()
    model_lidar.eval()
    model_thr.eval()
    ...
    for i, sample in enumerate(train_loader):
        ...
        pred, um  = net(x_lidar_1, x_lidar_2, x_lidar_3, x_lidar_4,x_thr_1, x_thr_2, x_thr_3, x_thr_4)
...
def test(test_loader, model_lidar, model_thr, net):
    net.eval()
    model_lidar.eval()
    model_thr.eval()
    totalNumber = 0
    ...
    with torch.no_grad():
        for i, sample in enumerate(test_loader):
            ...
            _,_,x_lidar_1, x_lidar_2, x_lidar_3, x_lidar_4 = model_lidar(lidar_thr_sd)
            _,_,x_thr_1, x_thr_2, x_thr_3, x_thr_4 = model_thr(thr)
            output, um = net(x_lidar_1, x_lidar_2, x_lidar_3, x_lidar_4,x_thr_1, x_thr_2, x_thr_3, x_thr_4)
