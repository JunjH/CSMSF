...
parser = argparse.ArgumentParser(description='training early fusion model for vision-lidar-thermal fusion')
...
parser.add_argument('--name', default='model_EF', type=str, help='name of experiment')
...
def main():
    ...
    encoder = modules.E_mvnet2_EF(original_model)
    model = net.model_EF(encoder, block_channel=[24, 32, 96, 160])
    model.to(device)
