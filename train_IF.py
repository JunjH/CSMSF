...
parser = argparse.ArgumentParser(description='training intermediate fusion model for vision-lidar-thermal fusion')
...
parser.add_argument('--name', default='model_IF', type=str, help='name of experiment')
...
def main():
    ...
    encoder_thr = copy.deepcopy(encoder_img)

    model = net.IF(encoder_img, encoder_lidar, encoder_thr, block_channel=[24, 32, 96, 160])
...
def train(train_loader, net, optimizer):
    ...
    for i, sample in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        pred, um  = net(img, lidar_thr_sd,thr)
