...
parser = argparse.ArgumentParser(description='training on vision-lidar-thermal')
...
parser.add_argument('--name', default='model_vlt', type=str, help='name of experiment')
...
def main():
    ...
    print('Number of parameters: {}'.format(sum([p.data.nelement() for p in model_img.parameters()])))
    print('Number of parameters: {}'.format(sum([p.data.nelement() for p in model_lidar.parameters()])))
    print('Number of parameters: {}'.format(sum([p.data.nelement() for p in model_thr.parameters()])))

    model = net.fusion_3sensors(block_channel=[24, 32, 96, 160])
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train(train_loader, model_img, model_lidar, model_thr, model, optimizer)
        total_score = test(test_loader, model_img, model_lidar, model_thr, model)
...
def train(train_loader, model_img, model_lidar, model_thr, net, optimizer):
    model_img.eval()
    model_lidar.eval()
    model_thr.eval()
    net.train()
    ...
    for i, sample in enumerate(train_loader):
        ...
        _,_,x_lidar_1, x_lidar_2, x_lidar_3, x_lidar_4 = model_lidar(lidar_thr_sd)
        _,_,x_thr_1, x_thr_2, x_thr_3, x_thr_4 = model_thr(thr)

        optimizer.zero_grad()
        pred, um  = net(x_img_1,x_img_2,x_img_3,x_img_4,x_lidar_1, x_lidar_2, x_lidar_3, x_lidar_4,x_thr_1, x_thr_2, x_thr_3, x_thr_4)
...
def test(test_loader, model_img, model_lidar, model_thr,  net):
    model_img.eval()
    model_lidar.eval()
    model_thr.eval()
    net.eval()
    ...
    with torch.no_grad():
        for i, sample in enumerate(test_loader):
            ...
            output, um = net(x_img_1,x_img_2,x_img_3,x_img_4,x_lidar_1, x_lidar_2, x_lidar_3, x_lidar_4,x_thr_1, x_thr_2, x_thr_3, x_thr_4)
