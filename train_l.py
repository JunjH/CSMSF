...
parser = argparse.ArgumentParser(description='training on lidar')
...
parser.add_argument('--name', default='model_l', type=str, help='name of experiment')
...
def main():
    ...
    encoder = modules.E_mvnet2_lidar(original_model)
...
def train(train_loader, net, optimizer):
    ...
    for i, sample in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        pred, um, _,_,_,_   = net(lidar_thr_sd)
...
def test(test_loader, net):
    ...
    with torch.no_grad():
        for i, sample in enumerate(test_loader):
            ...
            output, um,  _,_,_,_  = net(lidar_thr_sd)
