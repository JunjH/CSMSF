...
parser = argparse.ArgumentParser(description='training on vision-lidar')
...
parser.add_argument('--name', default='model_vl', type=str, help='name of experiment')
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train(train_loader, model_img, model_lidar, model, optimizer)
        total_score = test(test_loader, model_img, model_lidar, model)
...
def train(train_loader, model_img, model_lidar, net, optimizer):
    net.train()
    model_img.eval()
    model_lidar.eval()
    ...
    for i, sample in enumerate(train_loader):
        ...
        pred, um  = net(x_img_1,x_img_2,x_img_3,x_img_4,x_lidar_1, x_lidar_2, x_lidar_3, x_lidar_4)
...
def test(test_loader, model_img, model_lidar, net):
    net.eval()
    model_img.eval()
    model_lidar.eval()
    totalNumber = 0
    ...
    with torch.no_grad():
        for i, sample in enumerate(test_loader):
            ...
            _,_,x_img_1,x_img_2,x_img_3,x_img_4 = model_img(img)
            _,_,x_lidar_1, x_lidar_2, x_lidar_3, x_lidar_4 = model_lidar(lidar_thr_sd)
            output, um = net(x_img_1,x_img_2,x_img_3,x_img_4,x_lidar_1, x_lidar_2, x_lidar_3, x_lidar_4)
